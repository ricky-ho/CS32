...
bool pathExists(string maze[], int nRows, int nCols, int sr, int sc, int er, int ec)
{
    ...
    coordQueue.push(Coord(sr, sc));     // Push starting coordinate into the coord queue
    maze[sr][sc] = '#';                 // Update maze to indicate we have encountered it
    
    while(! coordQueue.empty())
    {
        Coord currentCoord = coordQueue.front();
        coordQueue.pop();               // Pop front coordinate off queue
        ...
        {
            coordQueue.push(Coord(row, col+1));        // Push the coordinate onto the stack
